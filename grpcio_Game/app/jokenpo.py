...
def Game_Start():
    ...
    MainMenu.iconbitmap(imagedir + "icon.ico")

    Label(MainMenu, text="JOKENPÔ", font=('Arial Black', 15)).pack(side=TOP, pady=10)

    photoPapel = PhotoImage(file=imagedir +"papel.png")
    photoimagePapel = photoPapel.subsample(4, 4)
    photoPedra = PhotoImage(file=imagedir +"pedra.png")
    photoimagePedra = photoPedra.subsample(4, 4)
    photoTesoura = PhotoImage(file=imagedir +"tesoura.png")
